...
ขั้นตอน deploy ระบบขึ้น production (VPS / Docker / Vercel)

> **สั้นที่สุด:** Next.js 16 + Prisma + PostgreSQL + เก็บไฟล์รูปบน disk → **แนะนำ VPS (DigitalOcean / Hetzner / Vultr)** เพราะ uploads ต้องการ persistent disk; ถ้าจะใช้ Vercel ต้องย้าย uploads ไป object storage ก่อน
...
|--------|----------|----------|
| **VPS (DigitalOcean Droplet / Hetzner)** ✅ แนะนำ | ใช้ `UPLOAD_DIR` เก็บรูปบน disk ได้เลย | ต้องดูแล server เอง (PM2 / systemd / nginx) |
...
ที่นี่จะอธิบาย flow **VPS (DigitalOcean Droplet)** เป็นหลัก ส่วน Docker จะแยกเป็นหัวข้อย่อย
...
- [ ] Postgres 14+ (DB เปล่า ๆ ชื่อ `mahamordo` หรือใช้ DB managed)
...
# PostgreSQL (ถ้าไม่ได้ใช้ managed DB)
sudo apt install -y postgresql postgresql-contrib
sudo -u postgres createuser --interactive --pwprompt mahamordo
sudo -u postgres createdb -O mahamordo mahamordo
...
```bash
# Database (local Postgres หรือ managed DB)
DATABASE_URL="postgresql://mahamordo:STRONG_PASSWORD@localhost:5432/mahamordo"
...
```bash
# Backup
pg_dump -U mahamordo -h localhost mahamordo > backup-$(date +%F).sql

# Restore
psql -U mahamordo -h localhost mahamordo < backup-2026-05-22.sql
```
...
```cron
0 3 * * * pg_dump -U mahamordo mahamordo | gzip > /backups/db-$(date +\%F).sql.gz && tar -czf /backups/uploads-$(date +\%F).tar.gz -C /var/lib/mahamordo uploads
```
...
services:
  ...
  db:
    image: postgres:16
    environment:
      POSTGRES_DB: mahamordo
      POSTGRES_USER: mahamordo
      POSTGRES_PASSWORD: STRONG_PASSWORD
    volumes:
      - mahamordo-db:/var/lib/postgresql/data
    restart: unless-stopped
...
ส่วน DB ใช้ Supabase Postgres หรือ Neon ได้ — แค่เปลี่ยน `DATABASE_URL`
